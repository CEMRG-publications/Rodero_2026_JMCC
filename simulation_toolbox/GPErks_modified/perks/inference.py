...
from GPErks_modified.utils.array import get_minmax, tensorize
...
class Inference:
    ...
    def summary(self):
        ...
        metrics_scores = list(
            m(tensorize(self.y_test), tensorize(self.y_pred_mean), tensorize(self.y_pred_std)).cpu()
            .numpy()
            if isinstance(m, IndependentStandardErrorMetric)
            else
