...
from GPErks_modified.gp.data.scaled_data import ScaledData
...
from GPErks_modified.train.early_stop import (
    EarlyStoppingCriterion,
    NoEarlyStoppingCriterion,
)
from GPErks_modified.train.snapshot import (
    EveryNEpochsSnapshottingCriterion,
    SnapshottingCriterion,
)
from GPErks_modified.train.train_stats import TrainStats, load_train_stats_from_file
from GPErks_modified.train.trainable import Trainable
from GPErks_modified.utils.array import tensorize
from GPErks_modified.utils.metrics import get_metric_name, IndependentStandardErrorMetric
...
class GPEmulator(Trainable):
    ...
    def _evaluate_metrics(self, X, y):
        ...
        return [m(y.cpu(), y_pred.cpu(), y_std.cpu()) if isinstance(m, IndependentStandardErrorMetric) else m(y_pred.cpu(), y.cpu()) for m in self.metrics]
